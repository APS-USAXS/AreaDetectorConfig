...
def write_template_file(tree, filename):
    '''write the tree to an XML file using lxml'''
    import lxml.etree
    xml_str = etree.tostring(tree.getroot())
    root = lxml.etree.fromstring(xml_str)
    xml_str = lxml.etree.tostring(root, pretty_print=True, encoding='UTF-8', xml_declaration=True, )
    f = open(filename, 'w')
    f.write(xml_str)
    f.write('\n')
...
# $Date: 2015-06-09 14:39:50 -0500 (Tue, 09 Jun 2015) $
# $Author: jemian $
# $Revision: 8473 $
# $URL: https://subversion.xray.aps.anl.gov/bcdaioc/9idc_AD/area_detector/SAXS_config/nexus_plugin/copy_attributes_to_template.py $
# $Id: copy_attributes_to_template.py 8473 2015-06-09 19:39:50Z jemian $
########### SVN repository information ###################
